...
func add_command(display_name: String, callable: Callable) -> void:
	...
	var key_name := "fusionex" + display_name.to_snake_case()
	cp.add_command("FusionEX/" + display_name, key_name, callable)
...
func create_extension() -> void:
	var out := []
	OS.execute("bash", ["addons/fusionex/editor_tools/create_gdextension.sh"], out, true)
	for o in out:
		print(o)
